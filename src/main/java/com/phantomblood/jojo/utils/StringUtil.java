...
//    }
     ...
     * @UpdateRemark: [当字符串的内容为undefined的时候则返回空字符串]
     */
...
//    }
    ...
    public static boolean isMobile(String s) {
        boolean rtn = validByRegex(
                "^((13[0-9])|(15[0|1|2|3|5|6|7|8|9])|14[5|7]|18[0-9]|17[0|6])[0-9]{8}$", s);
        return rtn;
    }
...
//    }
    ...
    public static String changeToBig(double value) {
        ...
        if (value < 0) {
            value = 0 - value;
            result = "（负数）" + doChangeToBig(value);
        } else {
            result = doChangeToBig(value);
        }
        return result;
    }

    public static String doChangeToBig(double value) {

        char[] hunit = {'拾', '佰', '仟'};                                               //段内位置表示
        char[] vunit = {'万', '亿'};                                                     //段名表示
        char[] digit = {'零', '壹', '贰', '叁', '肆', '伍', '陆', '柒', '捌', '玖'};  //数字表示
        long midVal = (long) (value * 100);                                  //转化为整形
        String valStr = String.valueOf(midVal);                                //转化成字符串
        int valStrs = valStr.length();
        String head = "";
        String rail = "";
        if (valStrs <= 1) {
            head = valStr.substring(0, valStr.length() - 1);               //取整数部分
            rail = valStr.substring(valStr.length() - 1);
        } else {
...
//    }
     ...
     * @param bfb
     * @return void
     * @Description: 百分比排名字符串分数转换
     * @author 孙崇宝
     * @Time 2015年12月21日
     */
    public static String getBfb(String bfb) {
        String[] drr = bfb.split("/");
        Double rr = Double.parseDouble(drr[0]);
        Double rr1 = Double.parseDouble(drr[1]);
        Double qq = rr / rr1;
        Double qq_1 = 1 - qq;
        Double qq_2 = qq_1 * 100;
        String qq_3 = getNumber(qq_2, null, 2, null);
     ...
     */
/*	public static boolean contain(String str, String searchStr,
			String argumentSeparator, boolean isIgnoreCase) {
		if (isEmpty(str))
			return false;
     ...
     * @return
     */
/*	public static boolean contain(String[] aryStr, String searchStr,
    ...
    public static int getTotalSize(String str) {
        ...
        return chineseCount + (letterCount + numCount) / 3 + blankCount / 4
                + otherCount * 3 / 4;
    }
    ...
    /**
     * 把一个字符串拆分后拼接成一个数组字符串返回
     * 例如initString=00000001000800140001，则返回retString=00000001,000000010008,0000000100080014
     *
     * @param initString
     * @return retString
     */
    public static String getSubStringArray(String initString) {
        int length = initString.length();
        String retString = "";
        if (length > 4) {
            retString = initString.substring(0, 4);
            for (int i = 2; i < length / 4; i++) {
                retString += "," + initString.substring(0, i * 4);
            }
        } else {
            retString = initString;
        }
        return retString;
     ...
     * @return
     */
    public static boolean istrueEmail(String s) {
     ...
     * @return
     */
    public static boolean mobileAndPhone(String s) {
        String check = "^(((17[0-9]|13[0-9]|14[5|7]|15[0|1|2|3|5|6|7|8|9]|18[0-9]|170)\\d{8})|((0\\d{2,3})?-?\\d{7,8}))$";
        Pattern pattern = Pattern.compile(check);
        Matcher match = pattern.matcher(s);
        if (match.matches() == false) {
            return false;
     ...
     * @param value
     * @return
     * @Method: notSpecialCharacter
     * @Description: 特殊字符校验(可包含#和@)
     */
    public static boolean notSpecialCharacter(String value) {
        boolean flag = true;
        String regEx = "[`~!$^&*+=|{}':;',\\[\\].<>/?~！￥……&*——+|{}【】‘；：”“’。，、？]";
